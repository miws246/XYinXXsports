...
p <- by_year |>
  ...
  labs(
    x = "Year",
    y = "Number of males",
    caption = glue("Note: {max_year} data is as of {data_cutoff} and is in light blue.")
  ) +
...
<font size="2"> `r glue("Note: {max_year} data is as of {data_cutoff} and is in light blue.")` </font>
...
p <- summ_top10 |>
  ...
  labs(
    ...
    y = "Number of events",
    fill = "Sport" # ,
    # caption = glue("{max_year} data is as of {data_cutoff}.")
  ) +
  scale_x_continuous(breaks = c(seq(1930, 2030, by = 10))) +
...
<font size="2"> Note: `r glue("{max_year} data is as of {data_cutoff}.")` </font>
...
p <- athletes_df |>
  ...
  geom_col(fill = bar_color, width = 0.75) +
  geom_text(aes(label = n), hjust = -0.1, size = 3) +
  theme_minimal() +
  theme(axis.title.y.left = element_text(angle = 0,
                                         vjust = 1.02,
                                         hjust = 1,
                                         margin = margin(r = -30, l = 10)
                                         )
        ) +
...
p <- all_results_sub |>
  ...
  scale_x_continuous(position = "top", limits = c(NA, 2600),
